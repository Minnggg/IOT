...
async function fetchPumpStatusData() {
    try {
        const response = await fetch('http://192.168.109.66:3000/auto/');
...
async function fetchSensorDataHumidity() {
    ...
    const statusUrl = "http://192.168.109.66:3000/auto";
    const pumpControlApiUrl = "http://192.168.109.66:3000/alert/sendMail";
    try {
        const response = await fetch(thinkSpeakApiUrl);
        const data = await response.json();
...
async function fetchSensorDataStatus() {
    const thinkSpeakApiUrl = "http://192.168.109.66:3000/auto";
...
function displayPumpStatus(pumpStatus) {
    console.log(`Trạng thái máy bơm: ${pumpStatus}`);
...
async function fetchTime() {
    try {
        const response = await fetch('http://192.168.109.66:3000/time/');
...
async function editTime(id, currentTime) {
    ...
    if (newTime && regex.test(newTime)) {
        try {
            const response = await fetch(`http://192.168.109.66:3000/time/edit/${id}`, {
                method: 'PATCH',
                headers: {
...
async function deleteTime(id) {
    try {
        ...
        const response = await fetch(`http://192.168.109.66:3000/time/delete/${id}`, {
            method: 'DELETE'
        });
